...
def availability_boxplot(input1):
    ...
    fig_layout = go.Layout(title='Availability Distribution for All Machines',
                             hovermode='closest', plot_bgcolor='#99ffcc', height=350, width=500)
...
fig_layout_1 = go.Layout(title='Available ECH Machines',
                   hovermode='closest', plot_bgcolor='#99ffcc', height=350, width=450)
fig_1 = go.Figure(data=[trace_1], layout=fig_layout_1 )
fig_1.update_layout(autosize=False, margin=dict(l=20,r=20,b=30,t=75,pad=4))
...
fig_2.update_layout(autosize=False, margin=dict(l=20,r=20,b=30,t=75,pad=4))


fig_3 = availability_boxplot(input1='All')
fig_3.update_layout(autosize=False, margin=dict(l=20,r=20,b=30,t=75,pad=4))
...
layout = html.Div([
    html.Div([html.A([html.Img(src='data:image/png;base64,{}'.format(encoded_image),style={'float':'left','width':'45px',
        ...
        html.H5("ECH SUMMARY",style={'padding-top':'15px','font-weight': 'bold',
                                                     'color': '#ffffff',
                                                     'text-align': 'center'}),
    ]),
...
# adding a plot
    ...
    '10px','height':'400px', 'width':'400px','display': 'inline-block'})],id='link',href='/apps/app3/1'),
            dcc.Link([dcc.Graph(
        id='plot2', figure=fig_2, style={'position':'absolute','left':
    '500px','height':'300px', 'width':'300px','display': 'inline-block'})],href='/apps/app3/2'),
            dcc.Link([dcc.Graph(
        id='plot3', figure=fig_3, style={'position':'absolute','left':
    '1000px','height':'300px', 'width':'400px','display': 'inline-block'})],href='/apps/app3/3')],style={'padding-top':'50px'})

    # html.P([dcc.Graph(id='plot2', figure=fig_2)],style={'float':'right','padding-right':
    #     '550px','height':'300px', 'width':'300px','display': 'inline-block'}),
    # html.P([dcc.Graph(id='plot3', figure=fig_3)],style={'float':'left','padding':
    #     '20px','height':'300px', 'width':'300px','display': 'inline-block'}),
])
